...
class LewtOpts < Hash
  ...
  def initialize ( extensions )
    ...
    if File.basename($0) != "lewt"
      puts "not on CL..."
    else
      puts "on CL..."
      parse_command_line( default_options )
    end

  end
  
  # handles parsing & translating options in command line mode
  def parse_command_line( native_options )
    options = self
    ...
    OptionParser.new do |opts|
      ...
      native_options.each do | name, details |
  ...
  # this function basically exists to get around the limited variable initializiation functionality of
